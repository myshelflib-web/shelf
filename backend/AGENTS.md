...
- **Jobs:** `ContentGenJob` holds a compact JSON `plan` plus a `cursor` — not one `ContentGenItem` per planned page up front. Item rows are created as each page starts. `plan` is cleared when the job finishes. Finished item rows older than 14 days are pruned; the job header stays for cost history. Dashboard polls job counts and lazy-loads item pages (`GET /content-gen/jobs/:id/items`). Overview packs omit topic lists. The syllabus catalog is compiled once per process; the job loop keeps compact slug refs, not full specs. The vector-index worker **skips ticks** while a gen job is in this process (pdf.js + 8k-token drafts together OOM a 512MB Render box).
