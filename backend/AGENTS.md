...
- **Output:** `publishGenerated.ts` writes `content.html` (reader) plus `content.txt` (source artifact) to the usual `admin/{subject}/{topic}/{article}/` prefix and upserts the `Article`. Generated subjects use their **own slugs** (per-exam subject slugs, `exam-briefs-*`) so `pruneStalePreloadedArticles` never touches them — see `generatedSubjectSlugs()`.
