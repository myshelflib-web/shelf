...
- **Provider:** `services/sarvam/` (OpenAI-compatible `chat/completions`). `SARVAM_API_KEY` selects Sarvam; unset falls back to the normal Shelf chat model via `generationChat()`. Thinking is **off by default** (`reasoning_effort: null`). It is turned on only when needed: unparseable first draft (retry at `medium` + 12k), syllabus/news revise after a failing audit, or when a completion comes back empty (client bumps `max_tokens` and enables `low` then `medium`). Rechecks, probes, and first-pass drafts stay off. Cost is estimated in **paise** from `sarvamPricing.ts`.
