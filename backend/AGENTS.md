...
- Indexing: `services/libraryIndex.ts` + `libraryIndexText.ts` — every published page type (PDF, HTML/MD/TXT/DOCX, sketch/doc notebooks, links) gets vector chunks. Corpus = catalog metadata + `content.html` / notes + highlights. If HTML is missing/thin, `libraryIndexPdf.ts` Range-GETs `source.pdf` (64KB chunks, never a full GetObject) and reads pdf.js **one PDF page at a time** until embed budget. Text pages are indexed as-is. Scanned pages (almost no text layer) are rasterized to a JPEG and OCR’d with Gemini **one page at a time** (`VECTOR_INDEX_OCR_PAGES`, default 12) — never the whole PDF as base64. OCR HTML is saved as `content.html` with `meta name="shelf-ocr"`. Files over `VECTOR_INDEX_PDF_MAX_BYTES` (default 64MB) are skipped as a last resort because pdf.js still allocates a buffer of the file length; typical 15–40MB textbooks are indexed. The worker indexes **one library page at a time**. Content hash prefix `v5:`. `scheduleIndexPage` after page create/update, highlight create/update/delete, PDF replace, and the processing callback; `purgePageVectors` on page delete.
