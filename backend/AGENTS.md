...
Health: `GET /health`, `GET /metrics`. JSON body limit 10mb. CORS from `CORS_ORIGIN` (comma-separated). Staging may set `ALLOW_VERCEL_PREVIEW_CORS=true` to allow `https://*.vercel.app` preview origins (leave off in production).
...
## Content generation (`services/contentGen/`)

Admin-triggered generation of **Learn** content. Routes: `routes/adminContentGen.ts` under `/api/admin/content-gen/*`; UI at `/admin/content`.

- **Provider:** `services/sarvam/` (OpenAI-compatible `chat/completions`). `SARVAM_API_KEY` selects Sarvam; unset falls back to the normal Shelf chat model via `generationChat()`. Thinking is **off by default**. Do **not** send `reasoning_effort: null` — JSON null is treated as the default (`medium`) and 105B thinks until `max_tokens` is gone (`finish_reason=length`, empty `content`, huge `reasoning_content`). Omit the key and send `chat_template_kwargs.enable_thinking: false`. Thinking is turned on only for unparseable-draft retry and revise-after-audit (`reasoning_effort: "medium"` + `enable_thinking: true`). Empty completions retry with thinking still off and a 16k budget. Cost is estimated in **paise** from `sarvamPricing.ts`.
- **Starter packs / syllabus library:** `syllabus/` is the dense catalog (exam → paper → subject → topic → leaf). `blueprints/` still hold the long-form chapter pages. `syllabus/index.ts` `specsForGoal()` merges them (blueprint slug wins). One leaf is one Learn page with a 4–5 point recheck checklist. Generate a whole goal or one `subjectSlug`. Pipeline: draft → audit for coverage, factual risk **and filler** → up to two revisions → publish. Score below `MIN_PUBLISH_SCORE` (70) is `SKIPPED` and the last draft is stored on the item. `POST /content-gen/jobs/:id/retry-failed` revises that draft (not a full redraft) and publishes when the score clears 70 and the job is not a dry run. Pages that never produced a draft still full-generate. Cost/storage: `costStorage.ts` (Sarvam 105B defaults ₹29.28 / ₹73.20 per 1M in/out; ~64 KB HTML+text per published page).
- **News briefs:** `contentGen/news/` clusters already-ingested `IngestItem` rows by title keywords, requires **2+ distinct publishers** per cluster, then synthesises an original brief and audits it for unsupported claims and copied phrasing. Only Shelf-written `shelfSummary` / `factualExcerpt` are read — never third-party article bodies. Every source is credited and linked in the output.
- **Output:** `publishGenerated.ts` writes `content.html` (reader) plus `content.txt` (source artifact) to the usual `admin/{subject}/{topic}/{article}/` prefix and upserts the `Article`. Generated subjects use their **own slugs** (per-exam subject slugs, `exam-briefs-*`) so `pruneStalePreloadedArticles` never touches them — see `generatedSubjectSlugs()`. Learn no longer seeds NCERT PDFs or official portal links; boot prune archives those rows and deletes their S3 objects. The personal library under `users/` is unchanged.
- **Diagrams, tables and callouts** are class-annotated HTML (`shelf-diagram*`, `shelf-doc-table`, `shelf-callout-*`), not SVG or inline styles — the reader sanitizer strips both. The body is wrapped in `<article class="shelf-generated">`; styles live in `frontend/src/app/generated-content.css`.
- **Jobs:** `ContentGenJob` holds a compact JSON `plan` plus a `cursor` — not one `ContentGenItem` per planned page up front. Item rows are created as each page starts. `plan` is cleared when the job finishes. Finished item rows older than 14 days are pruned; the job header stays for cost history. Dashboard polls job counts and lazy-loads item pages (`GET /content-gen/jobs/:id/items`). Overview packs omit topic lists. The syllabus catalog is compiled once per process; the job loop keeps compact slug refs, not full specs. The vector-index worker **skips ticks** while a gen job is in this process (pdf.js + drafts together OOM a 512MB Render box). **One job at a time**; inside a job, `CONTENT_GEN_CONCURRENCY` (default 2, max 3) generates that many pages in parallel. Paid Sarvam Pro still caps completions at 16k tokens — that is the product ceiling, not a free-tier throttle we invented.
- **Pause and resume:** `providerHealth.ts` separates provider **outage** (429/5xx/network/timeout) from **fatal** errors (401/403, missing key). On an outage `jobLoop.ts` returns the in-flight item to `QUEUED`, marks the job `PAUSED` and waits on a 30s→15m backoff probe, then continues from the same cursor. Fatal errors fail the job instead. `resumeJobs.ts` re-enters paused or restart-orphaned jobs — automatically on boot (`resumePendingContentGenJobs()` in `index.ts`) and on demand via `POST /content-gen/jobs/:id/resume`. A `RUNNING` job with **no remaining pages** is closed as `COMPLETED` (the last page was recorded, then the process died before `finishJob`). Resume does **not** redo `FAILED` or `SKIPPED` pages (the cursor already moved). `POST /content-gen/jobs/:id/retry-failed` starts a **new** job whose plan is those failed and below-score rows. `POST /content-gen/jobs/:id/stop` aborts in-flight Sarvam calls, skips leftover `QUEUED`/`RUNNING` items, and finishes the job (`COMPLETED` if every page is already scored, otherwise `FAILED` / `Stopped by admin`) so boot resume will not restart it. Published pages stay. `jobRegistry.ts` prevents a double run. News clusters live once on `ContentGenJob.plan` so a resumed run writes the briefs it originally planned.
