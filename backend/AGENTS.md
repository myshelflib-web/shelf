...
- **Provider:** `services/sarvam/` (OpenAI-compatible `chat/completions`). `SARVAM_API_KEY` selects Sarvam; unset falls back to the normal Shelf chat model via `generationChat()`. Cost is estimated in **paise** from `sarvamPricing.ts`.
