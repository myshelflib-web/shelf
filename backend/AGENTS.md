...
- Default chat: **`gemini-flash-lite-latest`** (Quick). Standard: **`gemini-flash-latest`**. Deep: pinned **`gemini-3.7-flash`**. Embeddings: **`gemini-embedding-001`** (auto-falls back from 002 if unset). See `services/shelfGeminiModels.ts`.
