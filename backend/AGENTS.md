...
- Indexing: `services/libraryIndex.ts` + `libraryIndexText.ts` — every published page type (PDF, HTML/MD/TXT/DOCX, sketch/doc notebooks, links) gets vector chunks. Corpus = catalog metadata + file text + highlight quotes/notes. Content hash prefix `v3:` so the worker refreshes older title-only rows. Thin/scanned PDFs (≤ `PDF_OCR_MAX_BYTES`, default 8MB) are OCR’d once via Gemini into `content.html` (`PDF_OCR=false` to disable). `scheduleIndexPage` after page create/update, highlight create/update/delete, PDF replace, and the processing callback; `purgePageVectors` on page delete.
- Background worker: `services/vectorIndexWorker.ts` polls for never-indexed, pre-`v3:` hash, or stale pages when vectors are configured (`VECTOR_INDEX_WORKER=false` to disable). One-shot backfill: `npm run vector:reindex -- 50`.
...
- Default chat: **`gemini-flash-lite-latest`** (Quick). Standard: **`gemini-flash-latest`**. Deep: pinned **`gemini-3.7-flash`**. Embeddings: pinned **`gemini-embedding-002`**. See `services/shelfGeminiModels.ts`.
...
- Embeddings default: **`gemini-embedding-002`** (native `batchEmbedContents`). Reindex after switching from 001. Pacing: `GEMINI_EMBED_RPM` (100), batch 8, 1s pause on paid tier.
