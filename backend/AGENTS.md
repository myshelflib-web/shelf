...
- Prompts: `services/goalPrompt.ts` + `goalTuning.ts` using `User.studyGoal`. Each track (UPSC, State PCS, Judiciary, CA, NEET PG, GATE) is tuned to official syllabus headings, standard material, and PYQ-style practice without inventing paper years. Answers must be structured Markdown; substantial replies can offer a one-line Try next. Library chat may call tools (`library_search`, `lookup_page`, `list_library`, `lookup_collection`, `lookup_recent_pages`, `lookup_starred`, `lookup_highlights`, `lookup_relevancy`, `lookup_planner`, `current_time`, `web_search`, `fetch_url`) when excerpts are thin.
