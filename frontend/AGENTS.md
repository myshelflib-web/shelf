...
| `/quiz` | Exam-style quizzes (MCQ, typed, photo working). Scope a document/collection/topic, upload notes + syllabus, or exam bank (PYQ/standard/preloaded). Settings: difficulty, timer, MCQ count, written count. Also `/quiz/[id]`. Same UI from nav, `g q`, `/quiz` slash, and search. |
