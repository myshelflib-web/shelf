...
| `/study-ai` | Full Study AI (multi-turn chat, RAG, tools, images, save/download). Also `/study-ai/[id]`. Per-thread **library scope** (collection/topic/page) and **syllabus / relevancy docs** (paste or upload; free 10 / paid 50). Stop generation, queue the next message, **edit a sent user message** (Cursor-style truncate + resubmit), delete a turn, **study flashcards** in a modal (download / save to library), and preview Mermaid diagrams. Type `/` for slash commands (`/quiz` opens the Quiz workspace, plus mind map, PYQ, flashcards, …); suggestion chips rotate above the composer. Tools can search the library, highlights, recents, starred pages, planner, Google, and public URLs, and can **create/update planner items** and **start quizzes**. Reader Ask answers general (off-document) questions too and shares the same write tools in free-form Ask. Reader chats are **saved** as page-scoped threads that appear here. Failed replies stay in the thread with short user-facing errors (no raw provider dumps). Library indexing covers PDFs, notebooks, docs, and imports; thin/scanned PDFs get OCR text when indexing. With no highlight, the panel sends this file's vectors plus a snapshot of the visible PDF page when the file has little text. |
| `/quiz` | Public, indexable explainer for guests; signed-in users get the quiz workspace (MCQ, typed, photo working). Tabs: **New quiz** and **Past quizzes**. Scope a document/collection/topic, upload notes + syllabus, or exam bank. Settings: difficulty, timer, MCQ/written counts, **proctored vs practice**. Proctored: fullscreen, centered questions; tab switch or leaving fullscreen submits and ends the paper. Practice: in-app, same centered paper, tabs allowed. Graded `/quiz/[id]` is a per-quiz **analysis board** (score, accuracy, time, topic/type breakdown, review). Home/take shells do not page-scroll; analysis does. `/quiz/[id]` is private (noindex). Same UI from nav, `g q`, `/quiz` slash, and search. |
| `/learn/...` | Public curriculum. Browse uses the Library explorer plus a library-style empty pane (greeting + catalog search; Study AI after sign-in). Articles open the reader. Save a copy into `/my-content` to edit. |
...
- Pages can be HTML, PDF (`PdfViewer`), YouTube (`VIDEO`), or link embed. Highlights: `HighlightToolbar` (colors); PDF pen has S/M/L size and auto-straightens strokes; click a highlight → note modal with **Remove highlight**. PDFs have no **Edit** (bottom bar or `e`). **Add page → YouTube** pastes a watch URL or playlist (playlist → topic, or a collection at library root). **Add page → Notebook** opens a multi-page sketch editor (draw-only ink + paste/upload images on the sheet, fixed A4 sheets, ruled/grid/blank paper, GoodNotes-style colors, + Page, pinch/Ctrl+scroll zoom). **Add page → Doc** opens a typed rich-text editor (headings, lists, fonts, colors, paste/upload images — no drawing). Legacy blank canvas pages (combined type+draw) still open via `LiveEditorRouter`. Imported HTML (TXT/MD/DOCX) still uses Edit for read/highlight vs edit.
