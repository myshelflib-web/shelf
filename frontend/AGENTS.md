...
| `/quiz` | Public, indexable explainer for guests; signed-in users get the quiz workspace (MCQ, typed, photo working). Scope a document/collection/topic, upload notes + syllabus, or exam bank (PYQ/standard/preloaded). Settings: difficulty, timer, MCQ count, written count. Sittings are **proctored**: fullscreen, centered questions; tab switch or leaving fullscreen submits and ends the paper. `/quiz/[id]` is private (noindex) and still requires sign-in. Same UI from nav, `g q`, `/quiz` slash, and search. |
