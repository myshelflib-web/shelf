...
Prod hooks (`RENDER_DEPLOY_HOOK_BACKEND`, etc.) stay for the manual production workflow.
...
- **PR with backend/worker changes** → shared staging images + staging Render hooks run automatically.
- **Merge to `main`** → production is **not** auto-deployed. Use Actions → Deploy production when you want to publish prod.
